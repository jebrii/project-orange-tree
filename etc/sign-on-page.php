...
<!-- SCRIPTS -->
  ...
  function isValid(value,fieldType) {

    switch (fieldType) {
      case "email":
        var pattern = new RegExp(/^[+A-z0-9._-]+@[a-zA-Z0-9.-]+\.[A-z]{2,5}$/);
        break;
      case "phone":
  ...
  // TODO: FIX; NOT WORKING FOR SOME REASON
        ...
        break;
      default:
        alert("Error: invalid fieldType passed to isValid function.");
    }
    return pattern.test(value);
  }

  function hasChars(value, charType) {
    ...
    switch (charType) {
      ...
      case "lower":
        ...
        break;
      default:
        alert("Error: invalid charType passed to hasChars function.");
    }
    return pattern.test(value);
  }
  function validateLoginForm() {
    var success = true;
    var userValue = $("#login-username-input").val();
    var passwordValue = $("#login-password-input").val();
  ...
  function validateSubmitForm() {
    var errorMessage = "Please fix issues with the following fields:";
    var showError = false;

    var emailValue = $("#signup-email-input").val();
    var userValue = $("#signup-username-input").val();
    var passwordValue = $("#signup-password-input").val();
    var confirmValue = $("#signup-confirm-input").val();

    // TODO: Turn this all into a for loop that iterates through a key:value object
    ...
    if ( userValue ) {
      $("#signup-username-error-0").css("display","none");
      if (userValue.length >= 6 && userValue.length <= 24) {
        $("#signup-username-error-1").css("display","none");
        if (isValid(userValue,"user")) {
          // TODO: CHECK TO SEE IF DUPLICATES A CURRENT USER
          /*
          var currentUsers = [];
          // Some code
          if (! currentUsers.includes(userValue)) {
            console.log("User valid");
            $(".username-error-3").css("display","none");
          } else {
            showError = true;
            errorMessage += "\n - Username";
            $("#signup-username-error-3").css("display","block");
          }
          */
        } else {
          showError = true;
          errorMessage += "\n - Username";
          $("#signup-username-error-2").css("display","block");
        }
      } else {
        showError = true;
        errorMessage += "\n - Username";
        $("#signup-username-error-1").css("display","block");
      }
    } else {
      $("#signup-username-error-0").css("display","block");
      errorMessage += "\n - Username";
    }

    // VALIDATE PASSWORD FIELD
    if (passwordValue.length >= 6 && passwordValue.length <= 24) {
      $("#signup-password-error-1").css("display","none");
      if ( isValid( passwordValue,"password" ) ) {
        $(".password-error").css("display","none");
      } else {
      showError = true;
      errorMessage += "\n - Password";
        var charTypes = ["num","upper","lower","special"];
        var errorNum = 1;
        for ( i in charTypes ) {
          errorNum ++;
          if ( hasChars( passwordValue, charTypes[i] ) ) {
            $("#signup-password-error-" + errorNum).css("display", "none");
          } else {
            $("#signup-password-error-" + errorNum).css("display", "block");
          }
        }
      }
    } else {
      showError = true;
      errorMessage += "\n - Password";
      $("#signup-password-error-1").css("display","block");
      $("#signup-password-error-2").css("display","none");
      $("#signup-password-error-3").css("display","none");
      $("#signup-password-error-4").css("display","none");
      $("#signup-password-error-5").css("display","none");
    }

  // VALIDATE CONFIRM FIELD
    if ( confirmValue ) {
      $("#signup-confirm-error-0").css("display","none");
      if ( confirmValue == passwordValue ) {
        $("#signup-confirm-error-1").css("display","none");
      } else {
        showError = true;
        errorMessage += "\n - Confirm Password";
        $("#signup-confirm-error-1").css("display","block");
      }
    } else {
      $("#signup-confirm-error-0").css("display","block");
      $("#signup-confirm-error-1").css("display","none");
      showError = true;
      errorMessage += "\n - Confirm Password";
    }
  ...
  $("#signup-reset-button").click( function () {
